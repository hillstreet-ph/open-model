...
| PostgreSQL schema | `open_model` |
| Sentry organization | `hillstreet` |
| Sentry project | `open-model` |
| GitHub repository | `hillstreet-ph/open-model` |
| Container image | `${DOCKERHUB_USERNAME}/open-model` |
...
7. Supabase migrations must target schema `open_model` and must be reviewed before production execution.
